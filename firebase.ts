...
export const verifyCredentials = async (email: string, password: string) => {
  try {
    ...
    if (cleanEmail === 'rajsjarma8@gmail.com' && password === 'FURY_ADMIN_2025') {
...
export const firebaseAwardReferral = async (referrerId: string) => {
  try {
    const cleanId = String(referrerId).trim();
    const ref = db.collection("users").doc(cleanId);
    await ref.update({
      diamonds: firebase.firestore.FieldValue.increment(20)
    });
    return { success: true };
  } catch (err: any) {
    handleFirestoreError(err);
  ...
  try {
    ...
    if (result && result.user) {
      ...
      const userData: any = {
        ...
        authMethod: 'google',
        isAdmin: user.email === 'rajsjarma8@gmail.com',
        joinedTournament: false,
        coins: 0,
        diamonds: 0,
        country: '',
      ...
      // Check if user already exists to preserve coins/diamonds
      if (!isSignup) {
        const existingData = userDoc.data();
        userData.coins = existingData.coins || 0;
        userData.diamonds = existingData.diamonds || 0;
        userData.isAdmin = existingData.isAdmin || userData.isAdmin;
        userData.joinedTournament = existingData.joinedTournament || false;
        userData.phone_number = existingData.phone_number || userData.phone_number;
        userData.country = existingData.country || '';
        userData.permanent_location = existingData.permanent_location || '';
      }

      await db.collection("users").doc(user.uid).set(userData, { merge: true });
      return { success: true, user: userData, isSignup };
    }
    return { success: false, error: "No user returned from Google Login" };
  } catch (err: any) {
    handleFirestoreError(err);
