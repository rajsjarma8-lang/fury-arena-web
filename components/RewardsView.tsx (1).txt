...
const RewardsView: React.FC<RewardsViewProps> = ({ user, redeemStore, t, onUpdateUser }) => {
  ...
  const handleCollectCoin = () => {
    if (isLimitReached || loading || cooldownTime > 0) return;
    setLoading(true);
    setIsClicked(true);
      ...
      <div className="bg-[#020617] border border-slate-800 rounded-[3rem] p-8 text-center relative overflow-hidden shadow-2xl">
        ...
        <div className="relative flex justify-center items-center mb-8">
          <button 
            ...
            disabled={isLimitReached || loading || cooldownTime > 0}
          ...
          >
            ...
            <span className={`text-[10px] font-black tracking-[0.1em] text-center leading-tight mt-1 ${!isLimitReached && cooldownTime === 0 ? 'text-slate-950' : 'text-slate-500'}`}>
              {isLimitReached ? 'LIMIT\nREACHED' : cooldownTime > 0 ? `WAIT ${cooldownTime}s` : loading ? 'MINING...' : 'MINE +10'}
            </span>
          </button>
          {!isLimitReached && cooldownTime === 0 && <div className="absolute w-24 h-24 bg-yellow-500 rounded-full blur-3xl opacity-30 animate-pulse"></div>}
        </div>
