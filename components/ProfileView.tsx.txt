...
const ProfileView: React.FC<ProfileViewProps> = ({ user, settings, onLogout, t, currentLang, onLangChange, onSupportClick }) => {
  ...
  const handleShare = () => {
    const shareText = `Play tournaments on Fury Arena and win real rewards! Sign up using my link and get started today! \n\n${referralLink}`;
        ...
        <div className="bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/20 rounded-3xl p-6 mt-6 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-4 mb-4 relative z-10">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-1">Get 5 <Gem className="w-3 h-3 inline text-cyan-400 mb-[2px]"/> for every friend!</p>
            </div>
          </div>
